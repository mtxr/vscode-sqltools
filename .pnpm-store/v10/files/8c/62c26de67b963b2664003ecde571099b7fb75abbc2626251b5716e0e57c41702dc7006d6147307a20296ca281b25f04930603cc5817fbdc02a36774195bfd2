...
function createProgram(fileName, overrideOptions) {
    ...
    return createProgram$2([fileName].concat(Array.from(dtsFiles)), compilerOptions, createCompilerHost(compilerOptions, true));
}
function createPrograms(input, overrideOptions) {
    ...
    for (let main of input) {
        ...
        if (options.dirName === dirName) {
            ...
            const program = createProgram$2(inputs.concat(Array.from(dtsFiles)), compilerOptions, host);
...
const plugin = (options = {}) => {
    ...
    function getModule(fileName) {
        ...
        if (!programs.length && fileName.endsWith(dts)) {
            ...
            if (!program && sys.fileExists(fileName)) {
