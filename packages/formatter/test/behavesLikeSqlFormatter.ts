 ...
 */
export default function behavesLikeSqlFormatter(language?: Config['language']) {
  const format = (query, cfg = {}) => sqlFormatter.format(query, { ...cfg, language });

  it('uses given indent config for indention', function () {
    const result = sqlFormatter.format('select count(*),Column1 FROM Table1;', { indent: '    ' });
  ...
  it('formats simple SET SCHEMA queries', function () {
    ...
    expect(result).toBe('SET SCHEMA tetrisdb;\n' + 'SET CURRENT SCHEMA bingodb;');
  });
  ...
  it('formats LIMIT of single value and OFFSET', function () {
    ...
    expect(result).toBe('LIMIT 5 OFFSET 8;');
  });
  ...
  it('preserves case of keywords', function () {
    ...
    expect(result).toBe('select distinct *\n' + 'frOM foo\n' + '  left join bar\n' + 'WHERe a > 1\n' + '  and b = 3');
  });
  ...
  it('keeps short parenthized list with nested parenthesis on single line', function () {
    ...
    expect(result).toBe('SELECT (a + b * (c - NOW()));');
  });
  ...
  it('formats simple DELETE query', function () {
    ...
    expect(result).toBe('DELETE FROM Customers\n' + "WHERE CustomerName = 'Alfred'\n" + '  AND Phone = 5002132;');
  });
  ...
  it('test tokenizer method exists', () => {
    ...
    expect(sqlFormatter.tokenize('SELECT *', { language })).toEqual([
      { type: 'reserved-top-level', value: 'SELECT' },
