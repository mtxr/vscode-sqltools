...
describe('StandardSqlFormatter', () => {
  ...
  it('formats long CREATE TABLE', () => {
    expect(format('CREATE TABLE items (a INT PRIMARY KEY, b TEXT, c INT NOT NULL, d INT NOT NULL);')).toBe(
      'CREATE TABLE items (\n' +
        '  a INT PRIMARY KEY,\n' +
        '  b TEXT,\n' +
        '  c INT NOT NULL,\n' +
  ...
  it('formats ALTER TABLE ... MODIFY query', () => {
    ...
    expect(result).toBe('ALTER TABLE supplier\n' + 'MODIFY supplier_name char(100) NOT NULL;');
  });
  ...
  it('recognizes mssql server @variables', () => {
    const result = format('SELECT @@SERVERNAME AS servername, @@SERVER_NAME AS server_name');
    expect(result).toBe('SELECT @@SERVERNAME AS servername,\n' + '  @@SERVER_NAME AS server_name');
  });
  ...
  it('recognizes %s placeholders', () => {
    const result = format('SELECT %s, %s, %s, %s, %d, %f FROM table WHERE id = %d;');
    expect(result).toBe(
      'SELECT %s,\n' + '  %s,\n' + '  %s,\n' + '  %s,\n' + '  %d,\n' + '  %f\n' + 'FROM table\n' + 'WHERE id = %d;'
    );
  });
  ...
  it('ignores words CASE and END inside other strings', () => {
    ...
    expect(result).toBe('SELECT CASEDATE,\n' + '  ENDDATE\n' + 'FROM table1;');
  });

  it('formats tricky line comments', () => {
    expect(format('SELECT a#comment, here\nFROM b--comment')).toBe('SELECT a #comment, here\n' + 'FROM b --comment');
  });
  ...
  it('Format query with cyrilic chars', () => {
    expect(
      format(`select t.column1 Кириллица_cyrilic_alias
...
from 注文
  ...
  it('Format query with dollar quoting', () => {
    expect(
      format(`create function foo() returns void AS $$
      begin
      select true;
      end;
      $$ language PLPGSQL;`)
...
where id = $1`
  ...
  it('Format query with returning as top level', () => {
    expect(format(`UPDATE "log" SET "time" = '2020-02-01 09:00:00' WHERE "id" = 1 RETURNING "time";`)).toEqual(
      dedent(`
        UPDATE "log"
  ...
  it('convert case reserved word to UPPER', () => {
    expect(
      format(
        `select case when external_sub_ref_id is null then 'a' else 'b' end as ref_1, case when subscription_ref_id is null then 'c' else 'd' end as ref_2, count(1) c from table1 group by 1, 2;`,
