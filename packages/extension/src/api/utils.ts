import { DOCS_ROOT_URL, VERSION } from '@sqltools/util/constants';
 ...
 */
function getlastRunInfo(): ILocalRunInfo {
  if (setup) {
    return setup.getContent();
  }
  setup = new SerializableStorage<any, string>(
    getConfigPath(RUNNING_INFO_FILENAME)
  );
  const localConfig = {
    current: <ILocalRunInfo>{
      numericVersion: numericVersion(VERSION),
      releaseNotes: `${DOCS_ROOT_URL}/changelog#v-${VERSION.replace(
        /^\s*(v(\d+\.\d+\.\d+|[^\s]+)).*/gi,
        '$1'
      ).replace(/[^\w]/g, '-')}`,
  ...
  try {
    ...
    localConfig.current.installedExtPlugins =
      localConfig.onDisk.installedExtPlugins;
    localConfig.current.updated =
      localConfig.current.numericVersion > localConfig.onDisk.numericVersion;
    localConfig.current.lastNotificationDate =
      localConfig.onDisk.lastNotificationDate || 0;
  } catch (e) {
    /**/
  }
...
function updateLastRunInfo(propsUpdater: Partial<ILocalRunInfo>) {
  if (!propsUpdater) return;
  try {
    const pastProps = getlastRunInfo();
    setup
      .content(<ILocalRunInfo>{
        ...(pastProps || {}),
        ...propsUpdater,
        installedExtPlugins: {
