...
import connectionStateCache, {
  ...
  LAST_USED_ID_KEY,
} from '../connection-manager/cache/connections-state.model';
import {
  DatabaseCompletionItem,
  TableColumnCompletionItem,
  TableCompletionItem,
} from './models';
...
export default class IntellisensePlugin<T extends ILanguageServer>
  implements ILanguageServerPlugin<T> {
  private server: T;

  private getQueryData = async (
    params: Arg0<Arg0<ILanguageServer['onCompletion']>>
  ) => {
  ...
  private getDatabasesCompletions = async ({
    currentWord,
    conn,
    suggestDatabases,
  }: {
    ...
    const dbs = (await conn.searchItems(
    ...
    if (dbs && dbs.length > 0) {
      return dbs.map(d => ({
        ...
        filterText: d.label.substring(
          d.label.toUpperCase().indexOf(currentWord)
        ),
  ...
  private getTableCompletions = async ({
    currentWord,
    conn,
    suggestTables,
  }: {
    ...
    const prefix =
      (suggestTables.prependQuestionMark ? '? ' : '') +
      (suggestTables.prependFrom ? 'FROM ' : '');
    const database =
      suggestTables.identifierChain && suggestTables.identifierChain[0].name;
    const suffix = suggestTables.appendDot ? '.' : '';

    const tables = (await conn.searchItems(ContextValue.TABLE, currentWord, {
      database,
    ...
    if (tables.length > 0) {
      return tables.map(t => ({
        ...
        // filterText is used to sort after the initial completion query
        filterText: t.label.substring(
          t.label.toUpperCase().indexOf(currentWord)
        ),
  ...
  private getColumnCompletions = async ({
    currentWord,
    conn,
    suggestColumns,
  }: {
    ...
    const tables = suggestColumns.tables
      ...
      .map(
        (t: [string]) =>
          <NSDatabase.ITable>{ label: t.pop(), database: t.pop() }
      );
    ...
    if (columns.length > 0) {
      return columns.map(c => ({
        ...TableColumnCompletionItem(c, {
          driver: conn.getDriver(),
          addTable: suggestColumns.tables.length > 1,
        }),
        // filterText is used to sort after the initial completion query
        filterText: c.label.substring(
          c.label.toUpperCase().indexOf(currentWord)
        ),
      }));
    }
    return [];
  ...
  private onCompletion: Arg0<
    ILanguageServer['onCompletion']
  > = async params => {
    ...
    try {
      ...
      const hueAst = sqlAutocompleteParser.parseSql(
        text.substring(0, currentOffset),
        text.substring(currentOffset)
      );

      completionsMap.query = (hueAst.suggestKeywords || [])
        .filter(kw => kw.value.startsWith(currentWord))
        .map(
          kw =>
            <CompletionItem>{
              label: kw.value,
              ...
              sortText: `${String(10000 - kw.weight).padStart(5, '0')}:${
                kw.value
              }`,
      ...
      const visitedKeywords: [string] = (hueAst.suggestKeywords || []).map(
        kw => kw.value
      );
      if (!conn) {
        log.info(
          'no active connection completions count: %d',
          completionsMap.query.length
        );
        return completionsMap.query;
      }
      // Can't distinguish functions types, so put all other keywords
      if (
        hueAst.suggestFunctions ||
        hueAst.suggestAggregateFunctions ||
        hueAst.suggestAnalyticFunctions
      ) {
        const staticCompletions = await conn.getStaticCompletions();
        for (const keyword in staticCompletions) {
          ...
          const value: NSDatabase.IStaticCompletion =
            staticCompletions[keyword];
          completionsMap.query.push({
            ...value,
            sortText: `4:${value.label}`,
      ...
      const [
        ...
        hueAst.suggestTables != undefined
          ? this.getTableCompletions({
              currentWord,
              conn,
              suggestTables: hueAst.suggestTables,
            })
          : [],
        hueAst.suggestColumns != undefined
          ? this.getColumnCompletions({
              currentWord,
              conn,
              suggestColumns: hueAst.suggestColumns,
            })
          : [],
        hueAst.suggestDatabases != undefined
          ? this.getDatabasesCompletions({
              currentWord,
              conn,
              suggestDatabases: hueAst.suggestDatabases,
            })
          : [],
      ]);
