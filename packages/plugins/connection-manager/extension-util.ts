...
export const getDriverSchemas = (
  ...
  let schema = {};
  let uiSchema = {};
  if (!driver) return prepareSchema(schema, uiSchema);
  try {
    schema =
      JSON.parse(
        fs
          .readFileSync(
            PluginResourcesMap.get<string>(
              buildResouceKey({
                type: 'driver',
                name: driver,
                resource: 'connection-schema',
              })
            )
          )
          .toString()
...
export const getInstalledDrivers = async (
  retry = 0
): Promise<SettingsScreenState['installedDrivers']> => {
  if (retry > 20) {
    log.info('stoped trying to find installed drivers.');
    return;
  }
  const driverExtensions: string[] =
    (
      Context.globalState.get<{ driver: string[] }>('extPlugins') || {
        driver: [],
      }
    ).driver || [];
  const installedDrivers: SettingsScreenState['installedDrivers'] = [];
  await Promise.all(
    driverExtensions.map(async id => {
      log.info(`getting extension %s information.`, id);
      const ext = await getExtension(id);
      log.info(
        `loaded extension information %s for driver %s.`,
        id,
        ext.driverName
      );
      if (ext && ext.driverAliases) {
        ext.driverAliases.map(({ displayName, value }) => {
          const iconsPath = PluginResourcesMap.get<IIcons>(
            buildResouceKey({ type: 'driver', name: value, resource: 'icons' })
          );
          let icon: string;
          if (iconsPath && iconsPath.default) {
            icon =
              'data:application/octet-stream;base64,' +
              fs.readFileSync(iconsPath.default).toString('base64');
          }
  ...
  if (installedDrivers.length === 0 && driverExtensions.length > 0) {
    ...
    return new Promise(res =>
      setTimeout(() => res(getInstalledDrivers(retry++)), 250)
    );
  }

  return installedDrivers.sort((a, b) =>
...
export const getExtension = async (
  id: string
): Promise<IDriverExtensionApi | null> => {
  try {
...
export const driverPluginExtension = async (driverName: string) => {
  const pluginExtenxionId = PluginResourcesMap.get(
    buildResouceKey({
      type: 'driver',
      name: driverName,
