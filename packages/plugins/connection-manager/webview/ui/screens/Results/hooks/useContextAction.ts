...
const getFileType = (choice: MenuActions) =>
  Object.values(MenuActions).includes(choice)
    ? choice === MenuActions.SaveJSONOption
      ? 'json'
      : 'csv'
    : undefined;

export const openMessagesConsole = () =>
  sendMessage(UIAction.CALL, {
    command: `${process.env.EXT_NAMESPACE}ViewConsoleMessages.focus`,
  });
...
export default function useContextAction() {
  ...
  const openResults = useCallback(
    (choice?: MenuActions.SaveCSVOption | MenuActions.SaveJSONOption | any) => {
      if (!result) return;
      sendMessage(UIAction.CALL, {
  ...
  const reRunQuery = useCallback(() => {
    ...
    if (queryType) {
      sendMessage(UIAction.CALL, {
        ...
        args: [
          queryParams,
          { ...options, page: page, pageSize: pageSize || 50 },
        ],
      });
