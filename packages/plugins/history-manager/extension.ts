...
export default class ConnectionManagerPlugin implements IExtensionPlugin {
  ...
  private addToHistoryHook = (
    evt: ICommandSuccessEvent<NSDatabase.IResult[]>
  ) => {
  ...
  private async historyMenu(prop = 'label'): Promise<string> {
    const items = this.explorer.getChildren().reduce<QuickPickItem[]>(
      (agg, group: HistoryTreeGroup) =>
        agg.concat(
    ...
    return await quickPick(items, prop, {
  ...
  private ext_runFromHistory = async (
    entry?: HistoryTreeItem
  ...
  public register(extension: IExtension) {
    ...
    hookedCommands.forEach(cmd =>
      extension.addAfterCommandSuccessHook(cmd, this.addToHistoryHook)
    );
    extension
      .registerCommand('runFromHistory', this.ext_runFromHistory)
