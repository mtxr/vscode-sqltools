...
export default class SQLToolsCodeLensProvider implements CodeLensProvider {
  ...
  async provideCodeLenses(document: TextDocument): Promise<CodeLens[]> {
    ...
    const defaultConn = extractConnName(
      document.getText(new Range(0, 0, 1, 0))
    );
    ...
    const allBlocks = text
      .replace(Constants.DELIMITER_START_REGEX, '<#####>$1')
      .split('<#####>');
    ...
    allBlocks.forEach(block => {
      ...
      const runCmd: Command = {
        arguments: [
          block.replace(Constants.DELIMITER_START_REPLACE_REGEX, '').trim(),
          (connName || defaultConn || '').trim() || undefined,
        ].filter(Boolean),
        title: `$(debug-start) Run on ${(
          connName ||
          defaultConn ||
          'active connection'
        ).trim()}`,
