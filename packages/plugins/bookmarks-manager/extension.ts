...
export default class BookmarksManagerPlugin implements IExtensionPlugin {
  ...
  private bookmarksMenu = async (): Promise<BookmarkTreeItem> => {
    ...
    return quickPick<BookmarkTreeItem>(items, 'item', {
...
-- @group {queryGroup}
-- @name {queryName}\n\n`
        .replace('{queryName}', item.name)
        .replace('{queryGroup}', item.parent.name);
  ...
  private ext_deleteBookmark = async (
  ...
  private ext_runFromBookmarks = async (
    item?: BookmarkTreeItem
  ): Promise<void> => {
    try {
      item = item || (await this.bookmarksMenu());
  ...
  public register(extension: IExtension) {
    ...
    Context.subscriptions.push(
      window.createTreeView(`${EXT_NAMESPACE}ViewBookmarksExplorer`, {
