...
export default {
  ...
  union all SELECT dbms_metadata.GET_DDL('INDEX', a.INDEX_NAME) AS DDL FROM user_indexes a WHERE table_name = :theobject
  union all select dbms_metadata.GET_DDL('TRIGGER', a.TRIGGER_NAME) FROM user_triggers a WHERE table_name = :theobject`,
  ...
  CASE WHEN a.type_name IS NOT NULL THEN a.type_owner || '.' || a.type_name ||
    DECODE(a.type_subname, NULL, '', '.' || a.type_subname) ELSE NVL(a.pls_type, a.data_type) END
