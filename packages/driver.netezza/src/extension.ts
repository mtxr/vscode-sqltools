...
export async function activate(extContext: ExtensionContext): Promise<IDriverExtensionApi> {
  ...
  const plugin: IExtensionPlugin = {
    ...
    async register(extension) {
      ...
      extension.resourcesMap().set(`driver/${DRIVER_ALIASES[0].value}/icons`, {
        active: extContext.asAbsolutePath('icons/netezza/active.png'),
        default: extContext.asAbsolutePath('icons/netezza/default.png'),
        inactive: extContext.asAbsolutePath('icons/netezza/inactive.png'),
      });
  ...
  return {
    ...
    parseBeforeSaveConnection: ({ connInfo }) => {
      ...
      connInfo.nzOptions = connInfo.nzOptions || {};
      if (Object.keys(connInfo.nzOptions).length === 0) {
        delete connInfo.nzOptions;
      }
    ...
    parseBeforeEditConnection: ({ connInfo }) => {
      ...
      formData.nzOptions = formData.nzOptions || {};
